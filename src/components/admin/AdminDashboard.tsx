...
//       try {
//         setLoading(true);
//         const response = await api.get('/admin/admin-stats', { withCredentials: true });
//         if (response.data.success) {
//           setStats({
//             enrollmentTimeline: response.data.data.enrollmentTimeline || [],
//             courseDistribution: response.data.data.courseDistribution || [],
//             totalInstructors: response.data.data.totalInstructors || 0,
...
//   // Render enrollment timeline chart
//   useEffect(() => {
...
//                     </p>
//                     <div className="flex space-x-2">
//                       <button className="px-3 py-1 bg-white text-gray-600 text-sm font-medium rounded border border-gray-200 hover:bg-gray-50">
//                         Previous
//                       </button>
...
const AdminDashboard: React.FC = () => {
  ...
  const [itemsPerPage] = useState<number>(5); // Number of courses per page

  // Fetch admin stats and course stats
  useEffect(() => {
    const fetchStats = async () => {
      try {
        setLoading(true);
  ...
  // Pagination logic for Course Statistics
