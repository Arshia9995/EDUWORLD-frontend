...
console.log(videoPreviewUrl,"video preview url")
  ...
  onError={(e) => {
    ...
    setVideoError('Error loading video. It may not be accessible or supported.');
  }}
...
//       setUploading(false);
//       return {
//         permanentUrl: data.downloadUrl, // Pre-signed URL for immediate use
//         key: data.imageUrl.split('.amazonaws.com/')[1], // Extract the S3 key
//       };
//     } catch (error: any) {
//       setUploading(false);
//       console.error('S3 upload error:', error.response?.data || error.message);
...
//     try {
//       setLoading(true);
//       const response = await api.get(`/users/updategetlessonbycourseid/${courseId}`, {
//         withCredentials: true,
//       });
...
//       if (fetchedLessons.length > 0) {
//         setSelectedLesson(fetchedLessons[0]);
//         setVideoPreviewUrl(fetchedLessons[0].video || null); // This is now a fresh pre-signed URL
//       } else {
//         setSelectedLesson(null);
//         setVideoPreviewUrl(null);
//       }
...
//     try {
//       let videoKey = selectedLesson.video; // This should now be the S3 key
//       let videoUrl = selectedLesson.video; // This will be the pre-signed URL for display
//       if (video) {
//         const { permanentUrl, key } = await uploadToS3(video);
//         videoKey = key; // Store the S3 key in the database
//         videoUrl = permanentUrl; // Use the pre-signed URL for immediate display
//         if (videoPreviewUrl && video) {
//           URL.revokeObjectURL(videoPreviewUrl);
//         }
...
//         title: values.title,
//         description: values.description,
//         video: videoKey, // Store the S3 key in the database
//         course: courseId,
//         duration: values.duration || undefined,
//       };
