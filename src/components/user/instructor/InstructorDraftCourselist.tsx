...
const InstructorDraftCourselist: React.FC = () => {
  ...
  const fetchDraftCourses = async () => {
    try {
      setLoading(true);
      const response = await api.get('/users/getdraftcourses', {
      ...
      if (response.status !== 200) {
        throw new Error(response.data.message || 'Failed to fetch draft courses');
      }
      ...
      const errorMessage = err.response?.data?.message || 'Failed to fetch draft courses';
  ...
  const handlePublishCourse = async (courseId: string) => {
    try {
      ...
      fetchDraftCourses(); // Refresh the draft courses list
    } catch (err: any) {
      ...
      >
        <div className="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 z-20 shadow-sm">
          <div className="flex items-center">
            ...
            <h1 className="text-2xl font-bold text-blue-900 hidden md:block">My Draft Courses</h1>
          </div>
        ...
        <div className="p-6 lg:p-8">
          <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-8 gap-4">
            <div>
              <h1 className="text-3xl font-extrabold text-blue-900 mb-2">My Draft Courses</h1>
              <p className="text-gray-600">Manage and edit all your unpublished course drafts</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-gray-500 text-sm">Total Draft Courses</p>
          ...
          {loading ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600">Loading your draft courses...</p>
            </div>
          ) : courses.length === 0 ? (
            <div className="bg-white p-8 rounded-xl shadow-sm border border-gray-100 max-w-2xl mx-auto text-center">
              ...
              <h3 className="text-xl font-bold text-gray-800 mb-2">No Draft Courses Yet</h3>
              <p className="text-gray-600 mb-6">
                {(searchTerm || filterOptions.category || filterOptions.priceRange || filterOptions.language) ?
                  'No draft courses match your search criteria. Try adjusting your filters or search term.' :
                  'Start creating a new course draft to prepare your content before publishing.'}
              </p>
              {(searchTerm || filterOptions.category || filterOptions.priceRange || filterOptions.language) ? (
                <button
                  onClick={clearFilters}
                ...
                >
                  Create Your First Draft Course
                </button>
              )}
            </div>
          ) : (
                    ...
                    <div className="p-5">
                      ...
                      <div className="absolute bottom-3 right-3 flex space-x-2">
                        <button
                          ...
                          className="flex items-center space-x-1 bg-yellow-500 text-white py-0.5 px-2 rounded-full hover:bg-yellow-600 transition-colors"
                          title="Add Lesson"
                        >
                          <FiFilePlus className="h-3 w-3" />
                        ...
                        <button
                          ...
                          className="flex items-center space-x-1 bg-blue-900 text-white py-0.5 px-2 rounded-full hover:bg-blue-800 transition-colors"
                          title="Edit Course"
                        >
                        ...
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            handlePublishCourse(course._id);
                          }}
...
export default InstructorDraftCourselist;